...
import { MowAlertAppearance, MowAlertType } from 'app/components/alert/alert.types'
...
export class MowAlertComponent implements OnChanges, OnInit, OnDestroy {
    ...
    @Input() appearance: MowAlertAppearance = 'soft'
    @Input() dismissed: boolean = false
    @Input() dismissible: boolean = false
    @Input() name: string = this._fuseUtilsService.randomId()
    @Input() showIcon: boolean = true
    @Input() type: MowAlertType = 'primary'
    @Input() timeout?: number
    @Output() readonly dismissedChanged: EventEmitter<boolean> = new EventEmitter<boolean>()

    timeRemaining$ = timer(0, 10).pipe(
        map((n) => {
            return Math.round(100 - n / this.timeout)
        }),
        takeWhile((n) => n >= 0),
        finalize(() => {
            if (this.timeout > 0) {
    ...
    constructor(
        ...
        private _mowAlertService: MowAlertService,
    ...
    ngOnInit(): void {
        ...
        this._mowAlertService.onDismiss
            .pipe(
